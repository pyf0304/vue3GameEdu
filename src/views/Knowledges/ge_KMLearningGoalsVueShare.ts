...
export const ge_KMLearningGoalsCache: { [key: string]: clsge_KMLearningGoalsENEx } = {};
...
export const Combinege_KMLearningGoalsCondition = async (): Promise<string> => {
  ...
  try {
    ...
    const strMsg: string = Format(
      '在组合查询条件(Combinege_KMLearningGoalsCondition)时出错!请联系管理员!{0}',
      objException,
    );
    throw strMsg;
  }
...
export const Combinege_KMLearningGoalsConditionObj = async (): Promise<clsge_KMLearningGoalsEN> => {
  ...
  let strWhereCond = ' 1 = 1 ';
  objge_KMLearningGoalsCond.SetCondFldValue(
    clsge_KMLearningGoalsEN.con_CourseId,
  ...
  try {
    ...
    const strMsg: string = Format(
      '在组合查询条件对象(Combinege_KMLearningGoalsConditionObj)时出错!请联系管理员!{0}',
      objException,
    );
    throw strMsg;
  }
 ...
 * @returns 条件串(strWhereCond)
 **/
export const Combinege_KMLearningGoalsConditionObj4ExportExcel =
  async (): Promise<clsge_KMLearningGoalsEN> => {
    //使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
    //例如 1 = 1 && UserName = '张三'
    ...
    try {
      ...
      const strMsg: string = Format(
        '在组合导出Excel条件对象(Combinege_KMLearningGoalsConditionObj4ExportExcel)时出错!请联系管理员!{0}',
        objException,
      );
      throw strMsg;
    }
...
export const BindTabByList = async (
  ...
  if (refge_KMLearningGoals_List.value != null)
    refge_KMLearningGoals_List.value.selectAllChecked = false;
};

export function ge_KMLearningGoals_DeleteKeyIdCache(
  strUserId: string,
  strKnowledgeModuleId: string,
): void {
  if (IsNullOrEmpty(strUserId) == false && IsNullOrEmpty(strKnowledgeModuleId) == false) {
    // 使用 delete 删除特定的键
    const cacheKey = `${strUserId}_${strKnowledgeModuleId}`;
    delete ge_KMLearningGoalsCache[cacheKey];
    return;
  }
}
