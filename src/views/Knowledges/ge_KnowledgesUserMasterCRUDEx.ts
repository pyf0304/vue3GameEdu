...
import {
  ...
  ge_KnowledgesUserMaster_GetObjLstBymIdLstAsync,
...
import { KnowledgeMasterLevel_BindDdl_MasterLevelIdInDivCache } from 'share-stu-study-base-lib';
import { QuestionTypeEx_BindDdl_QuestionTypeIdInDiv_CacheEx } from '@/ts/L3ForWApiEx/SystemSet/clsQuestionTypeExWApi';
import { ge_StuBatchQuesRelaEx_RecommendTopics0 } from '@/ts/L3ForWApiEx/InteractManage/clsge_StuBatchQuesRelaExWApi';
import {
  viewVarSet,
  divVarSet,
  CourseId_Session,
...
/* ge_KnowledgesUserMasterCRUDEx 的摘要说明。其中Q代表查询,U代表修改
 (AutoGCLib.WA_ViewScriptCSEx_TS4TypeScript:GeneCode)
*/
export default class ge_KnowledgesUserMasterCRUDEx
  extends ge_KnowledgesUserMasterCRUD
  implements IShowList
{
  public static vuebtn_Click: (strCommandName: string, strKeyId: any) => void;
  public static GetPropValue: (strPropName: string) => string;
  public static objPageCRUDEx: ge_KnowledgesUserMasterCRUDEx;
  public static divQuery_SelfPerceived: HTMLDivElement; //查询区的层对象
  ...
  public static btn_Click(strCommandName: string, strKeyId: string) {
    let objPage: ge_KnowledgesUserMasterCRUDEx;
    if (ge_KnowledgesUserMasterCRUDEx.objPageCRUDEx == null) {
      ge_KnowledgesUserMasterCRUDEx.objPageCRUDEx = new ge_KnowledgesUserMasterCRUDEx();
      objPage = <ge_KnowledgesUserMasterCRUDEx>ge_KnowledgesUserMasterCRUDEx.objPageCRUDEx;
    } else {
      objPage = <ge_KnowledgesUserMasterCRUDEx>ge_KnowledgesUserMasterCRUDEx.objPageCRUDEx;
    }
    ...
    switch (strCommandName) {
      ...
      case 'Query_SetSelfPerceived': //查询记录
        objPage.btnQuery_SelfPerceived_Click();
        break;
      ...
      default:
        strMsg =
          '命令:' +
          strCommandName +
          '在函数(ge_KnowledgesUserMasterCRUDEx.btn_Click)中没有被处理！';
        alert(strMsg);
        break;
    }
  }
...
*/
  ...
  public async BindDdl4QueryRegion() {
    // await this.SetDdl_MasterTypeIdInDiv(); //查询区域

    await this.SetDdl_MasterLevelIdInDiv(); //查询区域
    await this.SetDdl_QuestionTypeIdInDiv(); //查询区域
  }
  public async SetDdl_MasterLevelIdInDiv() {
    await KnowledgeMasterLevel_BindDdl_MasterLevelIdInDivCache(
      ge_KnowledgesUserMasterCRUDEx.divQuery_SelfPerceived,
      'ddlMasterLevelId_q',
    ); //
  }
  ...
  public async BindGv_ge_KnowledgesUserMaster4Func_Edit(divList: HTMLDivElement) {
    ...
    const strWhereCond = await this.Combinege_KnowledgesUserMasterConditionObj_Edit();
  ...
  public async BindGv_ge_KnowledgesUserMaster4Func_Goal(divList: HTMLDivElement) {
    ...
    const divDataLst = GetDivObjInDivObj(divList, 'divDataLst');
    const objge_KnowledgesUserMasterCond =
      await this.Combinege_KnowledgesUserMasterConditionObj_Goal();
    objge_KnowledgesUserMasterCond.SetCondFldValue(
      clsge_KnowledgesUserMasterEN.con_CourseId,
      CourseId_Session.value,
      '=',
    );
    const strWhereCond = JSON.stringify(objge_KnowledgesUserMasterCond);
    const intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex); //获取当前页
    let arrge_KnowledgesUserMasterExObjLst: Array<clsge_KnowledgesUserMasterENEx> = [];
    try {
      this.recCount = await ge_KnowledgesUserMaster_GetRecCountByCondAsync(strWhereCond);
      if (this.recCount == 0) {
        ...
        lblMsg.innerHTML = Format(
          '根据条件:[{0}]获取的对象列表数为0!',
          objge_KnowledgesUserMasterCond.whereCond,
        );
        if (divDataLst != null) {
          divDataLst.innerText = '';
          divDataLst.appendChild(lblMsg);
   ...
   **/
  public async Combinege_KnowledgesUserMasterConditionObj_Edit(): Promise<string> {
    //使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
    //例如 1 = 1 && UserName = '张三'
    const userStore = useUserStore();
    const strUserId = userStore.userId;
    let strWhereCond = ` 1 = 1 and ${clsge_KnowledgesUserMasterEN.con_UserId} = '${strUserId}'`;
    strWhereCond += ` and ${clsge_KnowledgesUserMasterEN.con_CourseId} = '${CourseId_Session.value}'`;
  ...
  public async btnQuery_SelfPerceived_Click() {
  ...
  public get knowledgeModuleId() {
    const strKnowledgeModuleId = ge_KnowledgesUserMasterCRUDEx.GetPropValue('knowledgeModuleId');
        ...
        funcName: (strKeyId: string, strText: string) => {
          ...
          (function (strKeyId: string) {
            btn1.onclick = function () {
              ge_KnowledgesUserMasterCRUDEx.vuebtn_Click('SetGoal', strKeyId);
            };
          })(strKeyId);
          return btn1;
        },
    ...
    const arrDataColumn: Array<clsDataColumn> = [
...

      {
        ...
        funcName: (strKeyId: string, strText: string) => {
          ...
          (function (strKeyId: string) {
            btn1.onclick = function () {
              ge_KnowledgesUserMasterCRUDEx.vuebtn_Click('SetSelfPerceived', strKeyId);
            };
          })(strKeyId);
          return btn1;
        },
  ...
  public async Combinege_KnowledgesUserMasterConditionObj(): Promise<clsge_KnowledgesUserMasterEN> {
    ...
    try {
      ...
      // if (this.masterTypeId_q != '' && this.masterTypeId_q != '0') {
      //   strWhereCond += Format(
      //     " And {0} = '{1}'",
      ...
      //   );
      // }
      // if (this.masterLevelId_q != '' && this.masterLevelId_q != '0') {
      //   strWhereCond += Format(
      //     " And {0} = '{1}'",
      //     clsge_KnowledgesUserMasterEN.con_MasterLevelId,
      //     this.masterLevelId_q,
      //   );
      //   objge_KnowledgesUserMasterCond.SetCondFldValue(
      //     clsge_KnowledgesUserMasterEN.con_MasterLevelId,
      //     this.masterLevelId_q,
  ...
  public get questionTypeId_q() {
    const strQuestionTypeId_q = ge_KnowledgesUserMasterCRUDEx.GetPropValue('questionTypeId_q');
