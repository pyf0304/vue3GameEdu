      ...
      <table>
        <tbody>
          <tr class="text-primary">
            ...
            <th @click="sortColumn('masterLevelName')">
              掌握等级
              <span v-if="sortColumnKey === 'masterLevelName'">
                <i :class="sortDirection === 'Asc' ? 'arrow-up' : 'arrow-down'"></i>
              </span>
            </th>

            <th @click="sortColumn('masterTypeName')">
              掌握类型名
              <span v-if="sortColumnKey === 'masterTypeName'">
                <i :class="sortDirection === 'Asc' ? 'arrow-up' : 'arrow-down'"></i>
              </span>
            </th>
          ...
          <tr v-for="(item, index) in items" :key="index" class="text-secondary">
            ...
            <td v-html="item.masterTypeName"></td>

            <td v-html="item.rightCount"></td>

            <td v-html="item.errorCount"></td>

            <td v-html="item.masteryValue"></td>
...
<script lang="ts">
  ...
  export default defineComponent({
    name: 'GeKnowledgesUserMasterList2',
    components: {
      // 组件注册
    },
