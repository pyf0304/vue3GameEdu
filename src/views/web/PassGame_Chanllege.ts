...
import { ge_GameLevelChapterRelaEx_GetObjLstByGameLevelIdEx } from '@/ts/L3ForWApiEx/GameLearn/clsge_GameLevelChapterRelaExWApi';
...
import { divVarSet, viewVarSet } from '@/views/web/PassGame_ChanllegeVueShare';
import { Questionnaire_GetObjLstAsync } from '@/ts/L3ForWApi/QuestionaireEdit/clsQuestionnaireWApi';

/* WApiUsers_QUDI_TSEx 的摘要说明。其中Q代表查询,U代表修改
 (AutoGCLib.WA_ViewScriptCSEx_TS4TypeScript:GeneCode)
*/
export class PassGame_Chanllege extends ge_UserCreditsLogCRUD implements IShowList {
  ...
  public get examBatchNo() {
    const strExamBatchNo = PassGame_Chanllege.GetPropValue('examBatchNo');
    return strExamBatchNo;
  }
  public set examBatchNo(value) {
    PassGame_Chanllege.vuebtn_Click('SetExamBatchNo', value);
  }
  ...
  public static btn_Click(strCommandName: string, strKeyId: string) {
    const objPage: PassGame_Chanllege = new PassGame_Chanllege();
    ...
    switch (strCommandName) {
      ...
      default:
        AccessBtnClickDefault(strCommandName, 'PassGame_Chanllege.btn_Click');
  ...
  public async ChallengeQuestionClick(
    ...
    PassGame_Chanllege.gameLevelId_Static = strGameLevelId;
    ...
    const arrge_GameLevelChapterRela = await ge_GameLevelChapterRelaEx_GetObjLstByGameLevelIdEx(
      strGameLevelId,
    );
    let intQuestionNum_CanSel = 0;
    for (const objInFor of arrge_GameLevelChapterRela) {
      const arrQuestionnaire_Sel = arrQuestionnaire.filter(
        (x) =>
          x.courseChapterId == objInFor.courseChapterId &&
          x.difficultyLevelId == strDifficultyLevelId &&
          x.questionTypeId == strQuestionTypeId,
      );
      intQuestionNum_CanSel += arrQuestionnaire_Sel.length;
    }
    ...
    try {
      ...
      if (
        ...
        const strWhereExam = Format(
          " gameLevelId='{0}' And examBatchNo='{1}' And UserId='{2}' And isRecommend = '0' ",
          strGameLevelId,
          strExamBatchNo,
          strUserId,
        );
      ...
      for (let i = 0; i < arrge_StuBatchQuesRelaObjLst.length; i++) {
        ...
        PassGame_Chanllege.objQuestions_Curr = objQuestionnaire_Curr;
        arrQuestions_Curr.push(objQuestionnaire_Curr);
      }
      PassGame_Chanllege.arrQuestions_Curr = arrQuestions_Curr;
      JSONstring = JSONstring.substring(0, JSONstring.length - 1); //去掉最后一个逗号问题；
      JSONstring += ']';
    ...
    PassGame_Chanllege.intActiveQuestion = 0;
    switch (strChallengeMode) {
      case enumChallengeMode.ViewLast_01:
        this.showQuestion_ViewLast(0, strExamBatchNo, PassGame_Chanllege.arrQuestions_Curr);
        break;
      case enumChallengeMode.StartGame_02:
        this.showQuestion_StartGame(0, strExamBatchNo, PassGame_Chanllege.arrQuestions_Curr);
        break;
      case enumChallengeMode.ContinuePass_03:
        this.showQuestion_ContinuePass(0, strExamBatchNo, PassGame_Chanllege.arrQuestions_Curr);
        break;
      default:
        const strMsg = Format(
          '挑战模式：{0}没有被处理！(in {1}.{2})',
    ...
    this.answerCard(PassGame_Chanllege.arrQuestions_Curr);
  }
  ...
  public async showQuestion_ViewLast(
    ...
    SetSpanHtmlInDivObj(divVarSet.refDivLayout, 'spnQuestioned', (id + 1).toString());
    let questioned = (id + 1) / arrQuestions.length;
    if (PassGame_Chanllege.intActiveQuestion != undefined) {
      $('#ques' + PassGame_Chanllege.intActiveQuestion)
    ...
    PassGame_Chanllege.intActiveQuestion = id;
    ...
    const strWhereStuExam = Format(
      "gameLevelId='{0}' And UserId='{1}' and ExamBatchNo='{2}' And isRecommend = '0' order by updDate desc",
      PassGame_Chanllege.gameLevelId_Static,
      strUserId,
      strExamBatchNo,
    );
    ...
    for (let i = 0; i < PassGame_Chanllege.arrCheckQues.length; i++) {
      if (PassGame_Chanllege.arrCheckQues[i].id == id) {
        $('#' + PassGame_Chanllege.arrCheckQues[i].item)
          .find('input')
          .prop('checked', 'checked');
        $('#' + PassGame_Chanllege.arrCheckQues[i].item).addClass('clickTrim');
        $('#ques' + PassGame_Chanllege.intActiveQuestion)
          .removeClass('question_id')
          .addClass('clickQue');
      }
  ...
  public async showQuestion_StartGame(
    ...
    //查询不是推荐的批次；
    const strWhereStuExam = Format(
      "gameLevelId='{0}' And UserId='{1}' And isRecommend = '0' order by updDate desc",
      PassGame_Chanllege.gameLevelId_Static,
      strUserId,
      strCourseId,
    );
  ...
  public async AddOrUpdUserAnswerResult(
    ...
    try {
      ...
      // 转化去掉首尾字符；
      const ArrayAnswer = PassGame_Chanllege.arrCheckQues;
      //声明类型变量参数，用来判断题型；
      let strType: string = '';
      switch (strQuestionTypeId) {
  ...
  public clickTrim(strCtrlId: string) {
    ...
    switch (type) {
      case 'checkbox':
        ...
        if (
          ...
          if ($('input[type=checkbox]:checked').length > 0) {
            // $("#ques" + PassGame_Chanllege.intActiveQuestion).removeClass("question_id").addClass("clickQue");//答题卡
          } else {
            $('#ques' + PassGame_Chanllege.intActiveQuestion)
              .removeClass('question_id')
          ...
          for (let i = 0; i < PassGame_Chanllege.arrCheckQues.length; i++) {
            //循环当前数组答案
            //判断如果Id相同时候
            //                        if (PassGame_Chanllege.arrCheckQues[i].id == PassGame_Chanllege.intActiveQuestion && PassGame_Chanllege.arrCheckQues[i].item == strCtrlId) {
            if (
              i == PassGame_Chanllege.intActiveQuestion &&
              PassGame_Chanllege.arrCheckQues[i].item == strCtrlId
            ) {
              //const strItem = "ItemNo";
              PassGame_Chanllege.arrCheckQues.splice(i, 1);
            }
          }
        } else {
          ////判断是否还有勾选的checkbox，如果没有那么去掉答题卡的颜色
          ...
          $('#ques' + PassGame_Chanllege.intActiveQuestion)
            ...
            .addClass('clickQue'); //答题卡
          if (
            PassGame_Chanllege.arrCheckQues.length == 0 ||
            (PassGame_Chanllege.intQuestion != PassGame_Chanllege.intActiveQuestion &&
              PassGame_Chanllege.intActiveQuestion != ques)
          ) {
            const check = new clsCheck();
            check.examId = examId ?? ''; //获取当前题目ID；
            check.id = PassGame_Chanllege.intActiveQuestion; //获取当前考题的编号
            check.item = strCtrlId; //获取当前考题的选项ID
            // check.answer =
            //   $('#' + strCtrlId)
            //     .find('input[name=item]:checked')
            //     .val() ?? ''; //获取当前考题的选项值
            check.answer = getCheckValueInDivObj(divVarSet.refDivLayout, strCtrlId).toString();
            check.type = type; //存放类型；
            PassGame_Chanllege.arrCheckQues.push(check);
          } else {
            //点击勾选checkbox 时候需要判断是否存在此值，不存在则需要插入；
            const arrCheckId = PassGame_Chanllege.arrCheckQues.map((x) => x.id);

            if (PassGame_Chanllege.arrCheckQues.indexOf(new clsCheck()) > -1) {
              //则包含该元素
            } else {
              let check = new clsCheck();
              check.examId = examId ?? 0; //获取当前题目ID；
              check.id = PassGame_Chanllege.intActiveQuestion; //获取当前考题顺序的编号
              check.item = strCtrlId; //获取当前考题的选项ID
              // check.answer =
              //   $('#' + strCtrlId)
              //     .find('input[name=item]:checked')
      ...
      case 'radio':
        ...
        for (let i = 0; i < PassGame_Chanllege.arrCheckQues.length; i++) {
          if (
            PassGame_Chanllege.arrCheckQues[i].id == PassGame_Chanllege.intActiveQuestion &&
            PassGame_Chanllege.arrCheckQues[i].item != strCtrlId
          ) {
            ques = PassGame_Chanllege.arrCheckQues[i].id;
            PassGame_Chanllege.arrCheckQues[i].item = strCtrlId; //获取当前考题的选项ID

            PassGame_Chanllege.arrCheckQues[i].answer = getCheckValueInDivObj(
        ...
        if (
          ...
          let check = new clsCheck();
          check.examId = examId ?? ''; //获取当前题目ID；
          check.id = PassGame_Chanllege.intActiveQuestion; //获取当前考题的顺序编号
          check.item = strCtrlId; //获取当前考题的选项ID

          const objCurrLi = GetLiObjInDivObj(divVarSet.refDivLayout, strCtrlId);
      ...
      if (y > 0) {
        ...
        for (const objInput of arrInput) {
          ...
          for (let i = 0; i < PassGame_Chanllege.arrCheckQues.length; i++) {
            ...
            if (
              PassGame_Chanllege.arrCheckQues[i].id == PassGame_Chanllege.intActiveQuestion &&
              PassGame_Chanllege.arrCheckQues[i].item == id
            ) {
              resultAnswer = 'yes'; //存在此问题的答案

              PassGame_Chanllege.arrCheckQues[i].answer = strText; //把当前答案赋值给数组；
            }
          }
          if (resultAnswer == '') {
            let check = new clsCheck();
  ...
  public async btnSubmitQuestions_Click() {
    ...
    const objQuestion = PassGame_Chanllege.arrQuestions_Curr[PassGame_Chanllege.intActiveQuestion];
    ...
    if (
      ...
      const userId = userStore.userId;
      const objPageGame = new PassGame_Chanllege();
      ...
      const strKeyId = PassGame_Chanllege.gameLevelId_Static;
  ...
  public async btnNextQuestion_Click() {
    ...
    //当前操作的题号
    if (PassGame_Chanllege.intActiveQuestion + 1 != PassGame_Chanllege.arrQuestions_Curr.length) {
      PassGame_Chanllege.intActiveQuestion = PassGame_Chanllege.intActiveQuestion + 1;
      switch (strChallengeMode) {
        case enumChallengeMode.ViewLast_01:
          await this.showQuestion_ViewLast(
            PassGame_Chanllege.intActiveQuestion,
            this.examBatchNo,
            PassGame_Chanllege.arrQuestions_Curr,
          );
          break;
        case enumChallengeMode.StartGame_02:
      ...
      alert('已经是最后一题');
      //showQuestion(PassGame_Chanllege.intActiveQuestion, questions)
    }
    //去掉填空题文本框样式
  ...
  public async btnUpQuestion_Click() {
    ...
    if (PassGame_Chanllege.arrCheckQues.length > 0) {
      //require(["../js/Web/PassGame_Chanllege.js"], function (index) {
      //上一题 值为1；
      const markType = '1';
    ...
    //当前操作题号，如果为0，则提示
    if (PassGame_Chanllege.intActiveQuestion == 0) {
      alert('已经是第一题');
    } else {
      switch (strChallengeMode) {
        case enumChallengeMode.ViewLast_01:
          this.showQuestion_ViewLast(
            PassGame_Chanllege.intActiveQuestion - 1,
            this.examBatchNo,
            PassGame_Chanllege.arrQuestions_Curr,
          );
          break;
  ...
  public async Getge_UserCreditsOrderByEduCls() {
    ...
    const ulPaperReportTreeBind = GetUlObjInDivObj(divVarSet.refDivLayout, 'PaperReportTreeBind');
  ...
  public async Show_GameChallenge() {
    ...
    let arrUserAnswerResultObjLst: Array<clsUserAnswerResultEN> = [];
    //关卡章节关系
    // const arrge_GameLevelChapterRelaObjLst: Array<clsge_GameLevelChapterRelaEN> = [];

    // const arrcc_CourseChapterObjLst: Array<clscc_CourseChapterEN_T> = [];
    const divGameLevelLst = <HTMLDivElement>document.getElementById('divGameLevelLst');
    try {
      //获取当前教学班 关卡
      arrge_GameLevelObjLst = await ge_GameLevel_GetObjLstAsync(strWhereCond);
      ...
      for (let i = 0; i < arrge_GameLevelExObjLst.length; i++) {
        ...
        if (objge_GameLevelEx.isRecommend == true) continue;
        //    //参数、用户、教学班、关卡号；
        ...
        const strQuestionNum = objge_GameLevelEx.questionNum;
        ...
        if (objge_StuBatchQuesRela != null) {
          ...
          const strWhereResult =
            "	examBatchNo='" + strExamBatchNo + "' And userId='" + strUserId + "'";
          ...
          if (
            ...
            const strWhereUpStuExam = Format(
              "gameLevelId='{0}' And UserId='{1}' And isSubmit = '1' And isRecommend = '0' order by updDate desc",
              strGameLevelId,
              strUserId,
            );
            const strLastExamBatchNo = await ge_StuBatchQuesRelaEx_GetLastExamBatchNo(
  ...
  public get challengeMode() {
    const strQuestionId = PassGame_Chanllege.GetPropValue('challengeMode');
    return strQuestionId;
  }
  public set challengeMode(strValue: string) {
    PassGame_Chanllege.vuebtn_Click('SetChallengeMode', strValue);
  }
...
function ChallengeQuestionClick(
  ...
  //把查看、继续做题、重新闯关参数，存入
  PassGame_Chanllege.vuebtn_Click('SetChallengeMode', strChallengeMode);
  ...
  if (strQuestionNum == 0) {
    ...
    switch (strQuestionTypeId) {
      case '02':
        ...
        objPage = new PassGame_Chanllege();
