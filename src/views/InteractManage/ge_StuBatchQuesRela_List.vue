      ...
      <table>
        <tbody>
          <tr class="text-primary">
            ...
            <th @click="sortColumn('gameLevelName')">
              关卡
              <span v-if="sortColumnKey === 'gameLevelName'">
                <i :class="sortDirection === 'Asc' ? 'arrow-up' : 'arrow-down'"></i>
              </span>
            </th>

            <th @click="sortColumn('questionTypeName')">
              题型
              <span v-if="sortColumnKey === 'questionTypeName'">
                <i :class="sortDirection === 'Asc' ? 'arrow-up' : 'arrow-down'"></i>
              </span>
            </th>

            <th @click="sortColumn('questionName')">
              题目名称
              <span v-if="sortColumnKey === 'questionName'">
                <i :class="sortDirection === 'Asc' ? 'arrow-up' : 'arrow-down'"></i>
              </span>
            </th>
            ...
            <th @click="sortColumn('answerModeName')">
              答案模式名称
              <span v-if="sortColumnKey === 'answerModeName'">
                <i :class="sortDirection === 'Asc' ? 'arrow-up' : 'arrow-down'"></i>
              </span>
            </th>
            ...
            <th @click="sortColumn('userId')">
              用户Id
              <span v-if="sortColumnKey === 'userId'">
                <i :class="sortDirection === 'Asc' ? 'arrow-up' : 'arrow-down'"></i>
              </span>
            </th>
            ...
            <th @click="sortColumn('optionName')">
              选项名称
              <span v-if="sortColumnKey === 'optionName'">
                <i :class="sortDirection === 'Asc' ? 'arrow-up' : 'arrow-down'"></i>
              </span>
            </th>
          </tr>
          ...
          <tr v-for="(item, index) in items" :key="index" class="text-secondary">
            <td>
              <input
                :id="'chk' + item.idStuBatchQuesRela"
            ...
            <td v-html="item.examBatchNo"></td>

            <td v-html="item.gameLevelName"></td>

            <td v-html="item.questionTypeName"></td>

            <td v-html="item.questionId"></td>

            <td v-html="item.chapterNameSim"></td>

            <td v-html="item.answerModeId"></td>

            <td v-html="item.userId"></td>

            <td v-html="item.userName"></td>

            <td v-html="item.isSubmit"></td>

            <td v-html="item.realFinishDate"></td>

            <td v-html="item.isMarking"></td>

            <td v-html="item.markDate"></td>

            <td v-html="item.operateTime"></td>

            <td v-html="item.updDate"></td>

            <td v-html="item.answerOptionId"></td>
          </tr> </tbody
      ></table>
    </template>
  </div>
