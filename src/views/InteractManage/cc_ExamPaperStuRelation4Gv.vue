      ...
      <table>
        <tbody>
          <tr class="text-primary">
            ...
            <th @click="sortColumn('examPaperName')">
              考卷
              <span v-if="sortColumnKey === 'examPaperName'">
                <i :class="sortDirection === 'Asc' ? 'arrow-up' : 'arrow-down'"></i>
              </span>
            </th>
          ...
          <tr v-for="(item, index) in items" :key="index" class="text-secondary">
            <td>
              <input
                :id="'chk' + item.idExamPaperStuRelation"
            ...
            <td v-html="item.courseExamPaperId"></td>

            <td v-html="item.idStudentInfo"></td>

            <td v-html="item.answerMultiOptions"></td>

            <td v-html="item.stuAnswerText"></td>
...
<script lang="ts">
  ...
  export default defineComponent({
    name: 'CcExamPaperStuRelation4Gv',
    components: {
      // 组件注册
    },
