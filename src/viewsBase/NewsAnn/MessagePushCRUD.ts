/**
 * 类名:MessagePushCRUD(界面:MessagePushCRUD,01120240)
 * 表名:MessagePush(01120280)
 * 版本:2025.02.27.1(服务器:WIN-SRV103-116)
 * 日期:2025/02/27 15:28:26
 * 生成者:
 工程名称:问卷调查(0112)
...
//import $ from "jquery";
import {
  CombineMessagePushCondition,
  divVarSet,
  viewVarSet,
  dataColumn,
  BindTabByList,
  refMessagePush_List,
} from '@/views/NewsAnn/MessagePushVueShare';
import {
...
export abstract class MessagePushCRUD implements clsOperateList {
  ...
  public async PageLoad() {
    ...
    try {
      ...
      if (viewVarSet.sortMessagePushBy == '')
        viewVarSet.sortMessagePushBy = `${clsMessagePushEN.con_MessagePushId} Asc`;
      //2、显示无条件的表内容在GridView中
      await this.BindGv_MessagePush(divVarSet.refDivList);
    } catch (e) {
  ...
  public async ExportExcel_MessagePush() {
    ...
    if (viewVarSet.sortMessagePushBy == null) {
      const strMsg = Format(
    ...
    const strWhereCond = await CombineMessagePushCondition();
  ...
  public async BindTab_MessagePush(
    ...
    if (refMessagePush_List.value != null) {
      dataColumn.value = arrDataColumn;
      await BindTabByList(arrMessagePushExObjLst, this.dispAllErrMsg_q);
    } else {
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_MessagePush函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
        arrMessagePushExObjLst,
        arrDataColumn,
  ...
  public async IndexPage(intPageIndex: number) {
    ...
    await this.BindGv_MessagePush(this.listPara.listDiv);
  }
  ...
  public async BindGv_MessagePush(divList: HTMLDivElement) {
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrMessagePushExObjLst, true);
        return;
      }
      ...
      const objPagerPara: stuPagerPara = {
        ...
        orderBy: viewVarSet.sortMessagePushBy, //如果该字段为空,就使用下面的排序函数
        sortFun: (x, y) => {
          console.log(x, y);
          return 0;
        },
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortMessagePushBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (clsMessagePushENEx.hasProperty(sortColumnKey)) {
      // 调用 SortColumn 函数
      this.SortColumn(sortColumnKey, sortDirection);
      return;
    }
    viewVarSet.sortMessagePushBy = sortBy;
