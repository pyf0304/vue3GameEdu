/**
 * 类名:QuestionnaireCRUD_List(界面:QuestionnaireCRUD_List,01120238)
 * 表名:Questionnaire(01120001)
 * 版本:2025.02.27.1(服务器:WIN-SRV103-116)
 * 日期:2025/02/27 15:29:03
 * 生成者:
 工程名称:问卷调查(0112)
...
/** QuestionnaireCRUD_List 的摘要说明。其中Q代表查询,U代表修改
 * (AutoGCLib.Vue_ViewScriptCS_TS4TypeScript:GeneCode)
 **/
export abstract class QuestionnaireCRUD_List implements clsOperateList {
  ...
  public static objPageCRUD: QuestionnaireCRUD_List;
  public static sortFunStatic: (ascOrDesc: string) => (x: any, y: any) => number;
  constructor() {
    this.listPara = new ListPara(divVarSet.refDivLayout, divVarSet.refDivList);
    QuestionnaireCRUD_List.objPageCRUD = this;
  ...
  public async PageLoad() {
    ...
    try {
      ...
      if (viewVarSet.sortQuestionnaireBy == '')
        viewVarSet.sortQuestionnaireBy = `${clsQuestionnaireEN.con_QuestionName} Asc`;
   ...
   **/
  public async BindTab_Questionnaire(
  ...
  public async BindGv_Questionnaire4Func(divList: HTMLDivElement) {
    ...
    if (viewVarSet.sortQuestionnaireBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortQuestionnaireBy)为空,请检查!(In BindGv_QuestionnaireCache)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortQuestionnaireBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    ...
    QuestionnaireCRUD_List.sortFunStatic = sortFun;
    await this.BindGv_Questionnaire4Func(this.listPara.listDiv);
  }
  ...
  public async CopyRecord(arrQuestionId: Array<string>) {
    ...
    try {
      const arrQuestionnaireObjLst = await Questionnaire_GetObjLstByQuestionIdLstAsync(
        arrQuestionId,
      );
