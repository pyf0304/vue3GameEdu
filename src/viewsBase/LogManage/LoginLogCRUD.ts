 ...
 **/
//import $ from "jquery";
import { CombineLoginLogCondition,divVarSet,viewVarSet,dataColumn,BindTabByList,refLoginLog_List } from "@/views/LogManage/LoginLogVueShare";
import { LoginLog_GetRecCountByCondAsync,LoginLog_GetObjLstAsync,LoginLog_DelRecordAsync,LoginLog_GetObjByLoginLogIdAsync,LoginLog_GetObjLstByLoginLogIdLstAsync,LoginLog_AddNewRecordAsync,LoginLog_DelLoginLogsAsync } from "@/ts/L3ForWApi/LogManage/clsLoginLogWApi";
...
import { LoginLogEx_FuncMapByFldName,LoginLogEx_GetObjExLstByPagerAsync } from "@/ts/L3ForWApiEx/LogManage/clsLoginLogExWApi";
...
if (viewVarSet.sortLoginLogBy == '') viewVarSet.sortLoginLogBy = `${ clsLoginLogEN.con_LoginLogId } Asc`;
...
{
const strMsg = Format("在显示列表时,排序字段(sortLoginLogBy)为空,请检查!(In BindGv_LoginLogCache)");
 ...
 **/
public async BindTab_LoginLog(divContainer: HTMLDivElement, arrLoginLogExObjLst: Array<clsLoginLogENEx>) 
{
...
await BindTab(divDataLst, arrLoginLogExObjLst, arrDataColumn,  clsLoginLogEN.con_LoginLogId, this); 
}
...
const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(objAnchorElement, viewVarSet.ascOrDesc4SortFun, viewVarSet.sortLoginLogBy, strSortExpress);
// 将 sortBy 按空格分成两部分
