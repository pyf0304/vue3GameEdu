 ...
 /**
 * 类名:gs_ReflectLog_Edit(界面:gs_ReflectLogCRUD,01120143)
 * 表名:gs_ReflectLog(01120702)
 * 版本:2025.02.27.1(服务器:WIN-SRV103-116)
 * 日期:2025/02/27 15:31:12
 * 生成者:
 工程名称:问卷调查(0112)
 ...
 **/
import $ from "jquery";
import { gs_ReflectLog_CheckPropertyNew,gs_ReflectLog_AddNewRecordAsync,gs_ReflectLog_GetUniCondStr,gs_ReflectLog_IsExistRecordAsync,gs_ReflectLog_GetUniCondStr4Update,gs_ReflectLog_AddNewRecordWithReturnKeyAsync,gs_ReflectLog_IsExistAsync,gs_ReflectLog_GetObjBymIdAsync,gs_ReflectLog_CheckProperty4Update,gs_ReflectLog_UpdateRecordAsync,gs_ReflectLog_EditRecordExAsync } from "@/ts/L3ForWApi/GradEduTopic/clsgs_ReflectLogWApi";
import { clsgs_ReflectLogEN } from "@/ts/L0Entity/GradEduTopic/clsgs_ReflectLogEN";
import { IsNullOrEmpty,Format } from "@/ts/PubFun/clsString";
import { gs_ReflectLog_DeleteKeyIdCache,divVarSet,refgs_ReflectLog_Edit } from "@/viewsShare/GradEduTopic/gs_ReflectLogVueShare";
...
{
 refgs_ReflectLog_Edit.value.hideDialog();
}
}
...
if (gs_ReflectLog_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
if (refgs_ReflectLog_Edit.value == null)
{
const strMsg = Format(
...
await refgs_ReflectLog_Edit.value.showDialog(this);
}
...
if (['02', '03', '06'].indexOf(clsgs_ReflectLogEN.PrimaryTypeId) > -1)
{
...
{
refgs_ReflectLog_Edit.value.hideDialog();
if (this.iShowList != null) this.iShowList.BindGvCache(clsgs_ReflectLogEN._CurrTabName, "");
}
}
else
{
...
if (this.iShowList != null) this.iShowList.BindGv(clsgs_ReflectLogEN._CurrTabName, this.keyId.toString());
}
}
break;
case "确认修改":
...
refgs_ReflectLog_Edit.value.Clear();
...
let objgs_ReflectLogEN;
try
{
objgs_ReflectLogEN = await refgs_ReflectLog_Edit.value.GetEditDatags_ReflectLogObj();
}
catch(e)
{
...
const strInfo = `添加[反思日志(gs_ReflectLog)]记录成功!`;
//显示信息框
...
const responseKeyId = await gs_ReflectLog_AddNewRecordWithReturnKeyAsync(objgs_ReflectLogEN);
const returnKeyId: string = responseKeyId;
if (IsNullOrEmpty(returnKeyId) == false)
{
objgs_ReflectLogEN.mId = Number(returnKeyId);
//gs_ReflectLog_ReFreshCache();
const strInfo = `添加[反思日志(gs_ReflectLog)]记录成功!`;
//显示信息框
if (this.isShowMsg == true) alert(strInfo);
...
console.error(strMsg);
        ...
        //3、用提供的关键字初始化一个类对象；
refgs_ReflectLog_Edit.value.ShowDataFromgs_ReflectLogObj(objgs_ReflectLogEN);
}
...
await refgs_ReflectLog_Edit.value.ShowDataFromgs_ReflectLogObj(objgs_ReflectLogEN);
...
const objgs_ReflectLogEN = await refgs_ReflectLog_Edit.value.GetEditDatags_ReflectLogObj();
...
{
//gs_ReflectLog_ReFreshCache();
//gs_ReflectLog_DeleteKeyIdCache(this.keyId);
}
return returnBool;
}
catch(e)
...
const returnBool = await gs_ReflectLog_EditRecordExAsync(objgs_ReflectLogEN);
if (returnBool == true)
{
//gs_ReflectLog_ReFreshCache();
}
...
refgs_ReflectLog_Edit.value.strCancelButtonText = value;
}
 /**
 * 获取按钮的标题
 **/
public  get btnSubmitgs_ReflectLog():string {
const strValue = refgs_ReflectLog_Edit.value.strSubmitButtonText;
return strValue;
}
 /**
...
refgs_ReflectLog_Edit.value.strSubmitButtonText = value;
}
}