/**
 * 类名:gs_ReflectLogCRUD(界面:gs_ReflectLogCRUD,01120143)
 * 表名:gs_ReflectLog(01120702)
 * 版本:2025.02.27.1(服务器:WIN-SRV103-116)
 * 日期:2025/02/27 15:28:41
 * 生成者:
 工程名称:问卷调查(0112)
...
} from '@/viewsShare/GradEduTopic/gs_ReflectLogVueShare';
import {
...
import {
  ...
  gs_ReflectLogEx_GetObjExLstByPagerAsync,
...
export abstract class gs_ReflectLogCRUD implements clsOperateList {
  ...
  public async PageLoad() {
    ...
    try {
      ...
      if (viewVarSet.sortgs_ReflectLogBy == '')
        viewVarSet.sortgs_ReflectLogBy = `${clsgs_ReflectLogEN.con_ReflectLogName} Asc`;
      //2、显示无条件的表内容在GridView中
      await this.BindGv_gs_ReflectLog4Func(divVarSet.refDivList);
    } catch (e) {
  ...
  public async ExportExcel_gs_ReflectLog() {
    ...
    if (viewVarSet.sortgs_ReflectLogBy == null) {
      const strMsg = Format(
    ...
    const strWhereCond = await Combinegs_ReflectLogCondition();
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_gs_ReflectLog();
  }
  ...
  public async BindTab_gs_ReflectLog(
    ...
    if (refgs_ReflectLog_List.value != null) {
      dataColumn.value = arrDataColumn;
      await BindTabByList(arrgs_ReflectLogExObjLst, this.dispAllErrMsg_q);
    } else {
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_gs_ReflectLog函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
  ...
  public async BindTab_gs_ReflectLog4Func(
    ...
    if (refgs_ReflectLog_List.value != null) {
      ...
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_gs_ReflectLog4Func函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
  ...
  public async IndexPage(intPageIndex: number) {
    ...
    await this.BindGv_gs_ReflectLog4Func(this.listPara.listDiv);
  }
  ...
  public async BindGv_gs_ReflectLog4Func(divList: HTMLDivElement) {
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrgs_ReflectLogExObjLst, true);
        return;
      }
      ...
      const objPagerPara: stuPagerPara = {
        ...
        orderBy: viewVarSet.sortgs_ReflectLogBy, //如果该字段为空,就使用下面的排序函数
        sortFun: (x, y) => {
          console.log(x, y);
          return 0;
        },
      };
      arrgs_ReflectLogExObjLst = await gs_ReflectLogEx_GetObjExLstByPagerAsync(objPagerPara);
    } catch (e) {
      const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortgs_ReflectLogBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (clsgs_ReflectLogENEx.hasProperty(sortColumnKey)) {
      // 调用 SortColumn 函数
      this.SortColumn(sortColumnKey, sortDirection);
      return;
    }
    viewVarSet.sortgs_ReflectLogBy = sortBy;
