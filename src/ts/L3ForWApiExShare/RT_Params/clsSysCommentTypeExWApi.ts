...
import { SysCommentType_GetObjLstCache,SysCommentType_GetObjLstByPagerAsync,SysCommentType_SortFunByKey } from "@/ts/L3ForWApi/RT_Params/clsSysCommentTypeWApi";
 ...
 * @returns 对象列表
*/
export  async function SysCommentTypeEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsSysCommentTypeENEx>> {
...
const arrSysCommentTypeExObjLst = arrSysCommentTypeObjLst.map(SysCommentTypeEx_CopyToEx);
 ...
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsSysCommentTypeEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
arrSysCommentTypeSel = arrSysCommentTypeSel.sort(SysCommentTypeEx_SortFunByKey(strSortFld, strSortType));
}
else {
