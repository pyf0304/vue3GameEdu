...
import { ge_ExaminationType_GetObjLstCache,ge_ExaminationType_GetObjLstByPagerAsync,ge_ExaminationType_SortFunByKey } from "@/ts/L3ForWApi/GameLearn/clsge_ExaminationTypeWApi";
 ...
 **/
export  function ge_ExaminationTypeEx_GetWebApiUrl(strController: string, strAction: string): string {
 ...
 **/
export  function ge_ExaminationTypeEx_CopyToEx(objge_ExaminationTypeENS:clsge_ExaminationTypeEN ): clsge_ExaminationTypeENEx
{
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, ge_ExaminationTypeEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function ge_ExaminationTypeEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsge_ExaminationTypeENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
const arrge_ExaminationTypeObjLst = await ge_ExaminationType_GetObjLstCache();
const arrge_ExaminationTypeExObjLst = arrge_ExaminationTypeObjLst.map(ge_ExaminationTypeEx_CopyToEx);
 const objSortInfo = GetSortExpressInfo(objPagerPara);
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsge_ExaminationTypeEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
 for (const objInFor of arrge_ExaminationTypeExObjLst) {
await ge_ExaminationTypeEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
 }
 }
...
const objge_ExaminationTypeCond = new clsge_ExaminationTypeENEx();
...
arrge_ExaminationTypeSel = arrge_ExaminationTypeSel.sort(ge_ExaminationTypeEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, ge_ExaminationTypeEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function ge_ExaminationTypeEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsge_ExaminationTypeENEx>> {
...
*/
export  function ge_ExaminationTypeEx_FuncMapByFldName(strFldName: string, objge_ExaminationTypeEx: clsge_ExaminationTypeENEx)
{
