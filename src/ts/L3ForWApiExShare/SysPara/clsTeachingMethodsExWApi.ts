...
import { TeachingMethods_GetObjLstCache,TeachingMethods_GetObjLstAsync,TeachingMethods_SortFunByKey } from "@/ts/L3ForWApi/SysPara/clsTeachingMethodsWApi";
 ...
 **/
export  function TeachingMethodsEx_CopyToEx(objTeachingMethodsENS:clsTeachingMethodsEN ): clsTeachingMethodsENEx
{
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, teachingMethodsEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function TeachingMethodsEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsTeachingMethodsENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
const arrTeachingMethodsObjLst = await TeachingMethods_GetObjLstCache();
const arrTeachingMethodsExObjLst = arrTeachingMethodsObjLst.map(TeachingMethodsEx_CopyToEx);
 const objSortInfo = GetSortExpressInfo(objPagerPara);
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsTeachingMethodsEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
const objTeachingMethodsCond = new clsTeachingMethodsENEx();
...
arrTeachingMethodsSel = arrTeachingMethodsSel.sort(TeachingMethodsEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, teachingMethodsEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function TeachingMethodsEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsTeachingMethodsENEx>> {
...
*/
export  function TeachingMethodsEx_FuncMapByFldName(strFldName: string, objTeachingMethodsEx: clsTeachingMethodsENEx)
{
