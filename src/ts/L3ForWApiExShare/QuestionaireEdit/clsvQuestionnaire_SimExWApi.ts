...
import { vQuestionnaire_Sim_GetObjLstCache,vQuestionnaire_Sim_GetObjLstByPagerAsync,vQuestionnaire_Sim_SortFunByKey } from "@/ts/L3ForWApi/QuestionaireEdit/clsvQuestionnaire_SimWApi";
 ...
 **/
export  function vQuestionnaire_SimEx_GetWebApiUrl(strController: string, strAction: string): string {
 ...
 **/
export  function vQuestionnaire_SimEx_CopyToEx(objvQuestionnaire_SimENS:clsvQuestionnaire_SimEN ): clsvQuestionnaire_SimENEx
{
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, vQuestionnaire_SimEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function vQuestionnaire_SimEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara , strCourseId:string):Promise<Array<clsvQuestionnaire_SimENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
const arrvQuestionnaire_SimObjLst = await vQuestionnaire_Sim_GetObjLstCache(strCourseId);
const arrvQuestionnaire_SimExObjLst = arrvQuestionnaire_SimObjLst.map(vQuestionnaire_SimEx_CopyToEx);
 const objSortInfo = GetSortExpressInfo(objPagerPara);
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsvQuestionnaire_SimEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
const objvQuestionnaire_SimCond = new clsvQuestionnaire_SimENEx();
...
arrvQuestionnaire_SimSel = arrvQuestionnaire_SimSel.sort(vQuestionnaire_SimEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, vQuestionnaire_SimEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function vQuestionnaire_SimEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsvQuestionnaire_SimENEx>> {
...
*/
export  function vQuestionnaire_SimEx_FuncMapByFldName(strFldName: string, objvQuestionnaire_SimEx: clsvQuestionnaire_SimENEx)
{
