...
import { ge_DifficultyLevel_GetObjLstCache,ge_DifficultyLevel_GetObjLstByPagerAsync,ge_DifficultyLevel_SortFunByKey } from "@/ts/L3ForWApi/SystemSet/clsge_DifficultyLevelWApi";
 ...
 **/
export  function ge_DifficultyLevelEx_GetWebApiUrl(strController: string, strAction: string): string {
 ...
 **/
export  function ge_DifficultyLevelEx_CopyToEx(objge_DifficultyLevelENS:clsge_DifficultyLevelEN ): clsge_DifficultyLevelENEx
{
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, ge_DifficultyLevelEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function ge_DifficultyLevelEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsge_DifficultyLevelENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
const arrge_DifficultyLevelObjLst = await ge_DifficultyLevel_GetObjLstCache();
const arrge_DifficultyLevelExObjLst = arrge_DifficultyLevelObjLst.map(ge_DifficultyLevelEx_CopyToEx);
 const objSortInfo = GetSortExpressInfo(objPagerPara);
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsge_DifficultyLevelEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
 for (const objInFor of arrge_DifficultyLevelExObjLst) {
await ge_DifficultyLevelEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
 }
 }
...
const objge_DifficultyLevelCond = new clsge_DifficultyLevelENEx();
...
arrge_DifficultyLevelSel = arrge_DifficultyLevelSel.sort(ge_DifficultyLevelEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, ge_DifficultyLevelEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function ge_DifficultyLevelEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsge_DifficultyLevelENEx>> {
...
*/
export  function ge_DifficultyLevelEx_FuncMapByFldName(strFldName: string, objge_DifficultyLevelEx: clsge_DifficultyLevelENEx)
{
