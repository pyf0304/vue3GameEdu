...
import { QuestionType_GetObjLstCache,QuestionType_GetObjLstByPagerAsync,QuestionType_SortFunByKey } from "@/ts/L3ForWApi/SystemSet/clsQuestionTypeWApi";
 ...
 **/
export  function QuestionTypeEx_CopyToEx(objQuestionTypeENS:clsQuestionTypeEN ): clsQuestionTypeENEx
{
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, questionTypeEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function QuestionTypeEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsQuestionTypeENEx>> {
...
const arrQuestionTypeExObjLst = arrQuestionTypeObjLst.map(QuestionTypeEx_CopyToEx);
 ...
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsQuestionTypeEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
 for (const objInFor of arrQuestionTypeExObjLst) {
await QuestionTypeEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
 }
 }
...
const objQuestionTypeCond = new clsQuestionTypeENEx();
...
arrQuestionTypeSel = arrQuestionTypeSel.sort(QuestionTypeEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, questionTypeEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function QuestionTypeEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsQuestionTypeENEx>> {
...
*/
export  function QuestionTypeEx_FuncMapByFldName(strFldName: string, objQuestionTypeEx: clsQuestionTypeENEx)
{
