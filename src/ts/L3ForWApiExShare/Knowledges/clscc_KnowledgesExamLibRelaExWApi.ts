...
export async function cc_KnowledgesExamLibRelaEx_GetObjExLstByPagerCache(
  ...
  const objcc_KnowledgesExamLibRela_Cond = new clscc_KnowledgesExamLibRelaENEx();
  ObjectAssign(objcc_KnowledgesExamLibRela_Cond, obj_Cond);
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      arrcc_KnowledgesExamLibRela_Sel = arrcc_KnowledgesExamLibRela_Sel.filter(
    ...
    if (arrcc_KnowledgesExamLibRela_Sel.length == 0) return arrcc_KnowledgesExamLibRela_Sel;
