...
import { vgs_KnowledgesLogic_User_Sim_GetObjLstAsync } from '@/ts/L3ForWApi/Knowledges/clsvgs_KnowledgesLogic_User_SimWApi';
import { vgs_KnowledgesLogic_User_Sim_SortFunByKey } from '@/ts/L3ForWApi/Knowledges/clsvgs_KnowledgesLogic_User_SimWApi';
import { Format } from '@/ts/PubFun/clsString';
import { vgs_KnowledgesLogic_User_Sim_FilterFunByKey } from '@/ts/L3ForWApi/Knowledges/clsvgs_KnowledgesLogic_User_SimWApi';
 ...
 * @returns 对象列表
 */
export async function vgs_KnowledgesLogic_User_SimEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
  strCourseId: string,
): Promise<Array<clsvgs_KnowledgesLogic_User_SimENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrvgs_KnowledgesLogic_User_SimObjLst = await vgs_KnowledgesLogic_User_Sim_GetObjLstCache(
    strCourseId,
  );
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrvgs_KnowledgesLogic_User_SimExObjLst) {
      const conFuncMap = await vgs_KnowledgesLogic_User_SimEx_FuncMapByFldName(
        objSortInfo.SortFld,
        objInFor,
      );
    }
  ...
  let arrvgs_KnowledgesLogic_User_Sim_Sel: Array<clsvgs_KnowledgesLogic_User_SimENEx> =
    arrvgs_KnowledgesLogic_User_SimExObjLst;
  const obj_Cond = JSON.parse(objPagerPara.whereCond);
  const objvgs_KnowledgesLogic_User_Sim_Cond = new clsvgs_KnowledgesLogic_User_SimENEx();
  ObjectAssign(objvgs_KnowledgesLogic_User_Sim_Cond, obj_Cond);
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      arrvgs_KnowledgesLogic_User_Sim_Sel = arrvgs_KnowledgesLogic_User_Sim_Sel.filter(
        (x) => x.GetFldValue(strKey) != null,
      );
      const strComparisonOp = dicFldComparisonOp[strKey];
      const strValue = objvgs_KnowledgesLogic_User_Sim_Cond.GetFldValue(strKey);
    ...
    if (arrvgs_KnowledgesLogic_User_Sim_Sel.length == 0) return arrvgs_KnowledgesLogic_User_Sim_Sel;
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvgs_KnowledgesLogic_User_Sim_Sel = arrvgs_KnowledgesLogic_User_Sim_Sel.sort(
    ...
    arrvgs_KnowledgesLogic_User_Sim_Sel = arrvgs_KnowledgesLogic_User_Sim_Sel.slice(
      intStart,
      intEnd,
    );
    return arrvgs_KnowledgesLogic_User_Sim_Sel;
  } catch (e) {
    const strMsg = Format(
...
export async function vgs_KnowledgesLogic_User_SimEx_GetObjExLstByPagerAsync(
  ...
  const arrvgs_KnowledgesLogic_User_SimObjLst = await vgs_KnowledgesLogic_User_Sim_GetObjLstAsync(
    objPagerPara.whereCond,
  );
  const arrvgs_KnowledgesLogic_User_SimExObjLst = arrvgs_KnowledgesLogic_User_SimObjLst.map(
    vgs_KnowledgesLogic_User_SimEx_CopyToEx,
 ...
 */
export async function vgs_KnowledgesLogic_User_SimEx_FilterFunByKey(strKey: string, value: any) {
  const strThisFuncName = 'FilterFunByKey';
  let strMsg = '';
  switch (strKey) {
    default:
      return vgs_KnowledgesLogic_User_Sim_FilterFunByKey(strKey, value);
  }
}
