...
import { cc_ExcellentType_GetObjLstCache,cc_ExcellentType_GetObjLstByPagerAsync,cc_ExcellentType_SortFunByKey } from "@/ts/L3ForWApi/CourseLearning/clscc_ExcellentTypeWApi";
...
export const cc_ExcellentTypeExController = "cc_ExcellentTypeExApi";
 ...
 * @returns 对象列表
*/
export  async function cc_ExcellentTypeEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clscc_ExcellentTypeENEx>> {
...
const arrcc_ExcellentTypeExObjLst = arrcc_ExcellentTypeObjLst.map(cc_ExcellentTypeEx_CopyToEx);
 ...
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clscc_ExcellentTypeEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
let arrcc_ExcellentTypeSel: Array < clscc_ExcellentTypeENEx > = arrcc_ExcellentTypeExObjLst;
const objCond = JSON.parse(objPagerPara.whereCond);
const objcc_ExcellentTypeCond = new clscc_ExcellentTypeENEx();
...
arrcc_ExcellentTypeSel = arrcc_ExcellentTypeSel.sort(cc_ExcellentTypeEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const arrcc_ExcellentTypeObjLst = await cc_ExcellentType_GetObjLstByPagerAsync(objPagerPara);
const arrcc_ExcellentTypeExObjLst = arrcc_ExcellentTypeObjLst.map(cc_ExcellentTypeEx_CopyToEx);
if (arrcc_ExcellentTypeExObjLst.length == 0) return arrcc_ExcellentTypeExObjLst;
let arrcc_ExcellentTypeSel: Array < clscc_ExcellentTypeENEx > = arrcc_ExcellentTypeExObjLst;
try {
