...
import { SourceType_GetObjLstCache,SourceType_GetObjLstByPagerAsync,SourceType_SortFunByKey } from "@/ts/L3ForWApi/InteractManage/clsSourceTypeWApi";
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, sourceTypeEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function SourceTypeEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsSourceTypeENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
const arrSourceTypeObjLst = await SourceType_GetObjLstCache();
const arrSourceTypeExObjLst = arrSourceTypeObjLst.map(SourceTypeEx_CopyToEx);
 const objSortInfo = GetSortExpressInfo(objPagerPara);
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsSourceTypeEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
 for (const objInFor of arrSourceTypeExObjLst) {
await SourceTypeEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
 }
 }
...
const objSourceTypeCond = new clsSourceTypeENEx();
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, sourceTypeEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function SourceTypeEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsSourceTypeENEx>> {
...
*/
export  function SourceTypeEx_FuncMapByFldName(strFldName: string, objSourceTypeEx: clsSourceTypeENEx)
{
