...
export class  clsBottleTypeEN extends clsGeneralTab
{
...
private mstrBottleTypeName = "";    //瓶子类型名
private mstrBottleTypeEnName = "";    //瓶子类型英文名
private mstrMemo = "";    //备注
...
/**
 * 瓶子类型名(说明:;字段类型:varchar;字段长度:30;是否可空:False)
 * (AutoGCLib.EntityLayer4TypeScript:Gen_EN_ClsProperty)
*/
 public SetBottleTypeName (value: string)
{
if (value  != undefined)
{
 this.bottleTypeName = value;
    this.hmProperty["bottleTypeName"] = true;
   this.sfUpdFldSetStr = this.updFldString;
}
}

/**
 * 瓶子类型英文名(说明:;字段类型:varchar;字段长度:30;是否可空:False)
 * (AutoGCLib.EntityLayer4TypeScript:Gen_EN_ClsProperty)
*/
 public SetBottleTypeEnName (value: string)
{
if (value  != undefined)
{
 this.bottleTypeEnName = value;
    this.hmProperty["bottleTypeEnName"] = true;
   this.sfUpdFldSetStr = this.updFldString;
}
}
...
break;
case clsBottleTypeEN.con_BottleTypeName:
this.bottleTypeName = strValue;
    this.hmProperty["bottleTypeName"] = true;
break;
case clsBottleTypeEN.con_BottleTypeEnName:
this.bottleTypeEnName = strValue;
    this.hmProperty["bottleTypeEnName"] = true;
break;
case clsBottleTypeEN.con_Memo:
this.memo = strValue;
...
break;
default:
strMsg = `字段名:[${strFldName}]在表对象:[BottleType]中不存在!(in ${this.constructor.name}.${strThisFuncName})`;
console.error(strMsg);
break;
}
}
...
//return propName in new clsBottleTypeEN();
const instance = new clsBottleTypeEN();
