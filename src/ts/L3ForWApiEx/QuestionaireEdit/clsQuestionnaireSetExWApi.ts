 ...
 * @returns 对象列表
 */
export async function QuestionnaireSetEx_GetObjExLstByPagerCache(
  objPagerPara: stuPagerPara,
): Promise<Array<clsQuestionnaireSetENEx>> {
  const strThisFuncName = 'GetObjLstByPagerCache';
  const arrQuestionnaireSetObjLst = await QuestionnaireSet_GetObjLstCache();
  ...
  const objQuestionnaireSet_Cond = new clsQuestionnaireSetENEx();
  ObjectAssign(objQuestionnaireSet_Cond, obj_Cond);
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      arrQuestionnaireSet_Sel = arrQuestionnaireSet_Sel.filter(
        (x) => x.GetFldValue(strKey) != null,
      );
      const strComparisonOp = dicFldComparisonOp[strKey];
      const strValue = objQuestionnaireSet_Cond.GetFldValue(strKey);
    ...
    if (arrQuestionnaireSet_Sel.length == 0) return arrQuestionnaireSet_Sel;
