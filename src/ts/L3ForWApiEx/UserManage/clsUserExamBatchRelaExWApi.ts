...
import {
  ...
  UserExamBatchRela_GetObjLstByPagerAsync,
 ...
 * @returns 对象列表
 */
export async function UserExamBatchRelaEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsUserExamBatchRelaENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrUserExamBatchRelaObjLst = await UserExamBatchRela_GetObjLstByPagerAsync(objPagerPara);
  const arrUserExamBatchRelaExObjLst = arrUserExamBatchRelaObjLst.map(UserExamBatchRelaEx_CopyToEx);
  if (arrUserExamBatchRelaExObjLst.length == 0) return arrUserExamBatchRelaExObjLst;
  let arrUserExamBatchRelaSel: Array<clsUserExamBatchRelaENEx> = arrUserExamBatchRelaExObjLst;
  try {
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrUserExamBatchRelaSel = arrUserExamBatchRelaSel.sort(
        UserExamBatchRelaEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
      //如果排序字段名[OrderBy]为空,就调用排序函数
      arrUserExamBatchRelaSel = arrUserExamBatchRelaSel.sort(objPagerPara.sortFun);
    }
    return arrUserExamBatchRelaSel;
  } catch (e) {
    const strMsg = Format(
  ...
  return new Array<clsUserExamBatchRelaENEx>();
}
