...
import { UserType_GetObjLstCache,UserType_GetObjLstByPagerAsync,UserType_SortFunByKey } from "@/ts/L3ForWApi/UserManage/clsUserTypeWApi";
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, userTypeEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function UserTypeEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsUserTypeENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
const arrUserTypeObjLst = await UserType_GetObjLstCache();
const arrUserTypeExObjLst = arrUserTypeObjLst.map(UserTypeEx_CopyToEx);
 const objSortInfo = GetSortExpressInfo(objPagerPara);
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsUserTypeEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
const objUserTypeCond = new clsUserTypeENEx();
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, userTypeEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function UserTypeEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsUserTypeENEx>> {
