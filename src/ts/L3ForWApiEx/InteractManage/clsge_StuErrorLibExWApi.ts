...
import { ge_StuErrorLib_GetObjLstAsync } from '@/ts/L3ForWApi/InteractManage/clsge_StuErrorLibWApi';
import { ge_StuErrorLib_SortFunByKey } from '@/ts/L3ForWApi/InteractManage/clsge_StuErrorLibWApi';
import { Format } from '@/ts/PubFun/clsString';
import { ge_StuErrorLib_FilterFunByKey } from '@/ts/L3ForWApi/InteractManage/clsge_StuErrorLibWApi';
import { clsSysPara4WebApi } from '@/ts/PubConfig/clsSysPara4WebApi';
export const ge_StuErrorLibEx_Controller = 'ge_StuErrorLibExApi';
...
export async function ge_StuErrorLibEx_GetObjExLstByPagerAsync(
  ...
  const arrge_StuErrorLibObjLst = await ge_StuErrorLib_GetObjLstAsync(objPagerPara.whereCond);
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrge_StuErrorLibExObjLst) {
      const conFuncMap = await ge_StuErrorLibEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  if (arrge_StuErrorLibExObjLst.length == 0) return arrge_StuErrorLibExObjLst;
  let arrge_StuErrorLib_Sel: Array<clsge_StuErrorLibENEx> = arrge_StuErrorLibExObjLst;
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrge_StuErrorLib_Sel = arrge_StuErrorLib_Sel.sort(
        ge_StuErrorLibEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
      //如果排序字段名[OrderBy]为空，就调用排序函数
      arrge_StuErrorLib_Sel = arrge_StuErrorLib_Sel.sort(objPagerPara.sortFun);
    }
    arrge_StuErrorLib_Sel = arrge_StuErrorLib_Sel.slice(intStart, intEnd);
    return arrge_StuErrorLib_Sel;
  } catch (e) {
    const strMsg = Format(
