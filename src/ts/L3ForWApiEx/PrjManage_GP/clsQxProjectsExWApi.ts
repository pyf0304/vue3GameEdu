...
import { QxProjects_GetObjLstCache,QxProjects_GetObjLstByPagerAsync,QxProjects_SortFunByKey } from "@/ts/L3ForWApi/PrjManage_GP/clsQxProjectsWApi";
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, qxProjectsEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function QxProjectsEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsQxProjectsENEx>> {
...
const arrQxProjectsExObjLst = arrQxProjectsObjLst.map(QxProjectsEx_CopyToEx);
 ...
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsQxProjectsEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, qxProjectsEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function QxProjectsEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsQxProjectsENEx>> {
...
*/
export  function QxProjectsEx_FuncMapByFldName(strFldName: string, objQxProjectsEx: clsQxProjectsENEx)
{
