...
import { RsStuType_GetObjLstCache,RsStuType_GetObjLstByPagerAsync,RsStuType_SortFunByKey } from "@/ts/L3ForWApi/EPortfolio/clsRsStuTypeWApi";
 ...
 * @returns 对象列表
*/
export  async function RsStuTypeEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsRsStuTypeENEx>> {
...
const arrRsStuTypeExObjLst = arrRsStuTypeObjLst.map(RsStuTypeEx_CopyToEx);
 ...
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsRsStuTypeEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
const objRsStuTypeCond = new clsRsStuTypeENEx();
