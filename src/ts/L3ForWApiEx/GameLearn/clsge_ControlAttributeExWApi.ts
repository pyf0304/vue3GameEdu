...
import { ge_ControlAttribute_GetObjLstAsync } from '@/ts/L3ForWApi/GameLearn/clsge_ControlAttributeWApi';
import { ge_ControlAttribute_SortFunByKey } from '@/ts/L3ForWApi/GameLearn/clsge_ControlAttributeWApi';
import { Format } from '@/ts/PubFun/clsString';
import { ge_ControlAttribute_FilterFunByKey } from '@/ts/L3ForWApi/GameLearn/clsge_ControlAttributeWApi';
...
export async function ge_ControlAttributeEx_GetObjExLstByPagerCache(
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrge_ControlAttributeExObjLst) {
      const conFuncMap = await ge_ControlAttributeEx_FuncMapByFldName(
        objSortInfo.SortFld,
        objInFor,
      );
    }
  ...
  let arrge_ControlAttribute_Sel: Array<clsge_ControlAttributeENEx> =
    arrge_ControlAttributeExObjLst;
  const obj_Cond = JSON.parse(objPagerPara.whereCond);
  const objge_ControlAttribute_Cond = new clsge_ControlAttributeENEx();
  ObjectAssign(objge_ControlAttribute_Cond, obj_Cond);
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      arrge_ControlAttribute_Sel = arrge_ControlAttribute_Sel.filter(
        (x) => x.GetFldValue(strKey) != null,
      );
      const strComparisonOp = dicFldComparisonOp[strKey];
      const strValue = objge_ControlAttribute_Cond.GetFldValue(strKey);
    ...
    if (arrge_ControlAttribute_Sel.length == 0) return arrge_ControlAttribute_Sel;
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrge_ControlAttribute_Sel = arrge_ControlAttribute_Sel.sort(
        ge_ControlAttributeEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
      //如果排序字段名[OrderBy]为空，就调用排序函数
      arrge_ControlAttribute_Sel = arrge_ControlAttribute_Sel.sort(objPagerPara.sortFun);
    }
    arrge_ControlAttribute_Sel = arrge_ControlAttribute_Sel.slice(intStart, intEnd);
    return arrge_ControlAttribute_Sel;
  } catch (e) {
    const strMsg = Format(
...
export async function ge_ControlAttributeEx_GetObjExLstByPagerAsync(
  ...
  const arrge_ControlAttributeObjLst = await ge_ControlAttribute_GetObjLstAsync(
    objPagerPara.whereCond,
  );
  const arrge_ControlAttributeExObjLst = arrge_ControlAttributeObjLst.map(
    ge_ControlAttributeEx_CopyToEx,
 ...
 */
export async function ge_ControlAttributeEx_FilterFunByKey(strKey: string, value: any) {
  const strThisFuncName = 'FilterFunByKey';
  let strMsg = '';
  switch (strKey) {
    default:
      return ge_ControlAttribute_FilterFunByKey(strKey, value);
  }
}
