 ...
 * @returns 对象列表
 */
export async function ge_OperationTypeEx_GetObjExLstByPagerCache(
  ...
  const objge_OperationType_Cond = new clsge_OperationTypeENEx();
  ObjectAssign(objge_OperationType_Cond, obj_Cond);
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      arrge_OperationType_Sel = arrge_OperationType_Sel.filter(
        (x) => x.GetFldValue(strKey) != null,
      );
      const strComparisonOp = dicFldComparisonOp[strKey];
      const strValue = objge_OperationType_Cond.GetFldValue(strKey);
    ...
    if (arrge_OperationType_Sel.length == 0) return arrge_OperationType_Sel;
