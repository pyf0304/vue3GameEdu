...
import { TextDirect_GetObjLstCache,TextDirect_GetObjLstByPagerAsync,TextDirect_SortFunByKey } from "@/ts/L3ForWApi/SysPara/clsTextDirectWApi";
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, textDirectEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function TextDirectEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsTextDirectENEx>> {
...
const arrTextDirectExObjLst = arrTextDirectObjLst.map(TextDirectEx_CopyToEx);
 ...
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsTextDirectEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, textDirectEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function TextDirectEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsTextDirectENEx>> {
...
*/
export  function TextDirectEx_FuncMapByFldName(strFldName: string, objTextDirectEx: clsTextDirectENEx)
{
