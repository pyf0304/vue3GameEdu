...
import { vQuestionNumByKnowledgeId_GetObjLstCache,vQuestionNumByKnowledgeId_GetObjLstByPagerAsync,vQuestionNumByKnowledgeId_SortFunByKey } from "@/ts/L3ForWApi/Knowledges/clsvQuestionNumByKnowledgeIdWApi";
 ...
 **/
export  function vQuestionNumByKnowledgeIdEx_GetWebApiUrl(strController: string, strAction: string): string {
 ...
 **/
export  function vQuestionNumByKnowledgeIdEx_CopyToEx(objvQuestionNumByKnowledgeIdENS:clsvQuestionNumByKnowledgeIdEN ): clsvQuestionNumByKnowledgeIdENEx
{
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, vQuestionNumByKnowledgeIdEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function vQuestionNumByKnowledgeIdEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara , strCourseId:string):Promise<Array<clsvQuestionNumByKnowledgeIdENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
const arrvQuestionNumByKnowledgeIdObjLst = await vQuestionNumByKnowledgeId_GetObjLstCache(strCourseId);
const arrvQuestionNumByKnowledgeIdExObjLst = arrvQuestionNumByKnowledgeIdObjLst.map(vQuestionNumByKnowledgeIdEx_CopyToEx);
 const objSortInfo = GetSortExpressInfo(objPagerPara);
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsvQuestionNumByKnowledgeIdEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
let arrvQuestionNumByKnowledgeIdSel: Array < clsvQuestionNumByKnowledgeIdENEx > = arrvQuestionNumByKnowledgeIdExObjLst;
...
arrvQuestionNumByKnowledgeIdSel = arrvQuestionNumByKnowledgeIdSel.sort(vQuestionNumByKnowledgeIdEx_SortFunByKey(objSortInfo.SortFld, objSortInfo.SortType));
}
else {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, vQuestionNumByKnowledgeIdEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function vQuestionNumByKnowledgeIdEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsvQuestionNumByKnowledgeIdENEx>> {
const strThisFuncName = "GetObjExLstByPagerAsync";
const arrvQuestionNumByKnowledgeIdObjLst = await vQuestionNumByKnowledgeId_GetObjLstByPagerAsync(objPagerPara);
const arrvQuestionNumByKnowledgeIdExObjLst = arrvQuestionNumByKnowledgeIdObjLst.map(vQuestionNumByKnowledgeIdEx_CopyToEx);
if (arrvQuestionNumByKnowledgeIdExObjLst.length == 0) return arrvQuestionNumByKnowledgeIdExObjLst;
let arrvQuestionNumByKnowledgeIdSel: Array < clsvQuestionNumByKnowledgeIdENEx > = arrvQuestionNumByKnowledgeIdExObjLst;
try {
...
arrvQuestionNumByKnowledgeIdSel = arrvQuestionNumByKnowledgeIdSel.sort(vQuestionNumByKnowledgeIdEx_SortFunByKey(strSortFld, strSortType));
}
else {
//如果排序字段名[OrderBy]为空,就调用排序函数
arrvQuestionNumByKnowledgeIdSel = arrvQuestionNumByKnowledgeIdSel.sort(objPagerPara.sortFun);
...
*/
export  function vQuestionNumByKnowledgeIdEx_FuncMapByFldName(strFldName: string, objvQuestionNumByKnowledgeIdEx: clsvQuestionNumByKnowledgeIdENEx)
{
