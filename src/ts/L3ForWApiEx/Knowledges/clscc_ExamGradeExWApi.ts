...
import { cc_ExamGrade_GetObjLstCache,cc_ExamGrade_GetObjLstByPagerAsync,cc_ExamGrade_SortFunByKey } from "@/ts/L3ForWApi/Knowledges/clscc_ExamGradeWApi";
 ...
 **/
export  function cc_ExamGradeEx_CopyToEx(objcc_ExamGradeENS:clscc_ExamGradeEN ): clscc_ExamGradeENEx
{
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, cc_ExamGradeEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function cc_ExamGradeEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clscc_ExamGradeENEx>> {
...
const arrcc_ExamGradeExObjLst = arrcc_ExamGradeObjLst.map(cc_ExamGradeEx_CopyToEx);
 ...
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clscc_ExamGradeEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
arrcc_ExamGradeSel = arrcc_ExamGradeSel.sort(cc_ExamGradeEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, cc_ExamGradeEx_ConstructorName, strThisFuncName);
console.error(strMsg);
 ...
 * @returns 对象列表
*/
export  async function cc_ExamGradeEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clscc_ExamGradeENEx>> {
...
*/
export  function cc_ExamGradeEx_FuncMapByFldName(strFldName: string, objcc_ExamGradeEx: clscc_ExamGradeENEx)
{
