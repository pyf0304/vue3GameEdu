...
import { gs_TotalDataStatistics_GetObjLstCache,gs_TotalDataStatistics_GetObjLstByPagerAsync,gs_TotalDataStatistics_SortFunByKey } from "@/ts/L3ForWApi/GradEduTools/clsgs_TotalDataStatisticsWApi";
 ...
 * @returns 对象列表
*/
export  async function gs_TotalDataStatisticsEx_GetObjExLstByPagerCache(objPagerPara: stuPagerPara , strIdCurrEduCls:string):Promise<Array<clsgs_TotalDataStatisticsENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
const arrgs_TotalDataStatisticsObjLst = await gs_TotalDataStatistics_GetObjLstCache(strIdCurrEduCls);
const arrgs_TotalDataStatisticsExObjLst = arrgs_TotalDataStatisticsObjLst.map(gs_TotalDataStatisticsEx_CopyToEx);
 const objSortInfo = GetSortExpressInfo(objPagerPara);
 if (IsNullOrEmpty(objSortInfo.SortFld) == false && clsgs_TotalDataStatisticsEN.AttributeName.indexOf(objSortInfo.SortFld) == -1)
 {
...
let arrgs_TotalDataStatisticsSel: Array < clsgs_TotalDataStatisticsENEx > = arrgs_TotalDataStatisticsExObjLst;
const objCond = JSON.parse(objPagerPara.whereCond);
const objgs_TotalDataStatisticsCond = new clsgs_TotalDataStatisticsENEx();
...
arrgs_TotalDataStatisticsSel = arrgs_TotalDataStatisticsSel.sort(gs_TotalDataStatisticsEx_SortFunByKey(strSortFld, strSortType));
}
else {
...
const arrgs_TotalDataStatisticsObjLst = await gs_TotalDataStatistics_GetObjLstByPagerAsync(objPagerPara);
const arrgs_TotalDataStatisticsExObjLst = arrgs_TotalDataStatisticsObjLst.map(gs_TotalDataStatisticsEx_CopyToEx);
if (arrgs_TotalDataStatisticsExObjLst.length == 0) return arrgs_TotalDataStatisticsExObjLst;
let arrgs_TotalDataStatisticsSel: Array < clsgs_TotalDataStatisticsENEx > = arrgs_TotalDataStatisticsExObjLst;
try {
