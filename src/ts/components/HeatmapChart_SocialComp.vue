<template>
  <div class="chart-container">
    <div ref="chart" class="chart" style="width: 700px; height: 350px"></div>
  </div>
</template>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const renderChart = (meData: number[], groupData: number[], xAxisData: string[]) => {
          ...
          series: [
            {
              ...
              renderItem: (params: any, api: any) => {
                ...
                // 限制正方形的宽度最大为 20px
                const meSize = Math.min(size[0], maxSize);
                return {
                  type: 'rect',
                  shape: {
                    x: position[0] - meSize / 2,
                    y: position[1] - meSize / 2,
                    width: meSize - 1,
                    height: meSize,
                  },
                  style: {
      ...
      onMounted(() => {
        const meData = props.meData; // [100, 74, 50, 29, 4, 28, 91, 94, 94, 72, 73, 100, 72, 97, 94, 0];
        // const compareData = [37, 10, 0, -14, -5, -6, 0, 18, 19, 14, 24, 35, 23, 0, 0, 0];
        const groupData = props.groupData; // [63, 64, 50, 43, 9, 34, 91, 76, 75, 58, 49, 65, 49, 97, 94, 0];
        ...
        //   'PPT美化',
        // ];
        renderChart(meData, groupData, xAxisData);
      });
      watch(
        () => [props.meData, props.groupData, props.xAxisData],
        () => {
